define([
  ...
  describe('DOMNode', function () {
    ...
    it('should toggle a class via toggleClass()', function () {
      var f = new DOMNode('.fixture');

      f.toggleClass('new');
      expect(f.hasClass('new')).toBeTruthy();
    ...
    it('should return true if the node is in the DOM via isInDOM()', function () {
      var f = new DOMNode(fixture);

      fixture.appendChild(document.createElement('div'));

      var div = f.one('div');
      expect(div.isInDOM()).toBeTruthy();

      fixture.innerHTML = '';
      expect(div.isInDOM()).toBeFalsy();
    });


    xit('should remove a node from the dom after a destroy()', function () {
      var f = new DOMNode(fixture);
