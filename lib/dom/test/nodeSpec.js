define([
  ...
  describe('DOMNode', function () {
    ...
    it('should return the current child index via getChildIndex()', function () {
      var f = new DOMNode('.fixture');
      ...
      expect(f.one('div').getChildIndex()).toEqual(0);
      expect(f.one('span').getChildIndex()).toEqual(1);
      expect(f.one('p').getChildIndex()).toEqual(2);

      expect(function () {
        f.one('ul').getChildIndex();
      }).toThrowError();
    });
