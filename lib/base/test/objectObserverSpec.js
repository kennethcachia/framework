define(['lib/base/object-observer'], function (ObjectObserver) {
  ...
  describe('ObjectObserver', function () {
    ...
    it('should fire an event after an update', function () {
      o.a = 100;
      observer.sync();
      expect(callback).toHaveBeenCalledWith([{}, {}, {a: 100}]);

      o.x = 50;
      observer.sync();
      expect(callback).toHaveBeenCalledWith([{x: 50}, {}, {}]);

      delete o.x;
      observer.sync();
      expect(callback).toHaveBeenCalledWith([{}, {x: undefined}, {}]);
    });
    ...
    it('should ignore changes if discard() is called', function () {
      o.a = 100;
      observer.discard();
      observer.sync();
      expect(callback).not.toHaveBeenCalled();
