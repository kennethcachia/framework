...
define(['lib/base/check'], function (Check) {
  ...
  var Assert = {
    ...
    /**
     * Asserts that it's an array.
     *
     * @method
     * @param {*} check The entity to be checked.
     * @throws Will throw an error if not an array.
     */
    isArray: function (check) {
      if (!Check.isArray(check)) {
        this._throwError('Not an array');
      }
    },
    ...
    /**
     * Asserts that it's a number.
     *
     * @method
     * @param {*} check The entity to be checked.
     * @throws Will throw an error if not a number.
     */
    isNumber: function (check) {
      if (!Check.isNumber(check)) {
        this._throwError('Not a number');
      }
    },


    /**
     * Asserts that it's a boolean.
     *
     * @method
     * @param {*} check The entity to be checked.
     * @throws Will throw an error if not a boolean.
     */
    isBoolean: function (check) {
      if (!Check.isBoolean(check)) {
        this._throwError('Not a boolean');
      }
    },
