...
define([
   ...
   * @param {String} eventName The event name.
   * @param {Object=} data
   *        Additional data that will be passed on
   *        to all the event callbacks.
   */
  Eventable.prototype.fire = function (eventName, data) {
    if (Check.not.isUndefined(data)) {
      Assert.isObject(data);
    }

    this._listeners.executeAll(this.BEFORE + eventName, data);
    this._listeners.executeAll(eventName, data);
    this._listeners.executeAll(this.AFTER + eventName, data);
  };
