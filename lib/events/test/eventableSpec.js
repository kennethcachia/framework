define(['lib/events/eventable'], function (Eventable) {
  ...
  describe('Eventable', function () {
    ...
    it('should allow the removal of callbacks added using before() or after() by using the before- or after- suffix', function () {
      ...
      expect(e._listeners._callbacks['before-event']).not.toBeUndefined();
      expect(e._listeners._callbacks['after-event']).not.toBeUndefined();

      e.off('before-event', callbackA);
      e.off('after-event', callbackB);

      expect(e._listeners._callbacks['before-event']).toBeUndefined();
      expect(e._listeners._callbacks['after-event']).toBeUndefined();
    });
  });
